...
const ACTION_RE = /<drink-water-action>([^<]+)<\/drink-water-action>/;
...
function parseAction(promptText) {
  ...
  if (trimmed === '/drink-water') return { kind: 'ack' };
  if (trimmed === '/drink-water-status') return { kind: 'status' };
  const snooze = trimmed.match(/^\/drink-water-snooze(?:\s+(\d+))?$/);
  if (snooze) {
...
function performAction(action, state, now) {
  if (action.kind === 'ack') {
    return {
      ...
      contextText:
        '<drink-water-confirmation>\n' +
        'The user acknowledged drinking water. Timer has been reset.\n' +
  ...
  if (action.kind === 'snooze') {
    ...
    return {
      ...
      contextText:
        '<drink-water-confirmation>\n' +
        'The user snoozed water reminders for ' + action.minutes + ' minutes.\n' +
